...
class Generator(tf.keras.Model):
    def __init__(self, conf):
        ...
        block_latent = [layers.Linear(input_dim=conf['latent_dim'],
        ...
        self.block_latent = tf.keras.Sequential(
            block_latent, name='block_latent')
        self.block_combined = tf.keras.Sequential(
            block_combined, name='block_combined')
        self.linear_last = layers.Linear(units=conf['input_size']**2 * conf['channel'],
